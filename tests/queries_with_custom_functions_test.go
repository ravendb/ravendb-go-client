...
func queriesWithCustomFunctionsQueryCmpXchgWhere(t *testing.T, driver *RavenTestDriver) {
	...
	op := ravendb.NewPutCompareExchangeValueOperation("Tom", "Jerry", 0)
	err = store.Operations().Send(op, nil)
	assert.NoError(t, err)
	op = ravendb.NewPutCompareExchangeValueOperation("Hera", "Zeus", 0)
	err = store.Operations().Send(op, nil)
	assert.NoError(t, err)
	op = ravendb.NewPutCompareExchangeValueOperation("Gaya", "Uranus", 0)
	err = store.Operations().Send(op, nil)
	assert.NoError(t, err)
	op = ravendb.NewPutCompareExchangeValueOperation("Jerry@gmail.com", "users/2", 0)
	err = store.Operations().Send(op, nil)
	assert.NoError(t, err)
	op = ravendb.NewPutCompareExchangeValueOperation("Zeus@gmail.com", "users/1", 0)
	err = store.Operations().Send(op, nil)
	assert.NoError(t, err)
